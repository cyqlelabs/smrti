...
When an agent calls `remember()`, Smrti embeds the text and stores it as a typed graph node (concept, belief, episode, or goal) carrying a Bayesian truth value, an attention weight, and an emotional valence score. Every observation is appended to an immutable evidence log — truth values are primarily updated through evidence merges, with direct confidence mutations reserved for policy operations (contradiction resolution and forget). In all server modes, Smrti automatically extracts entity/claim structure from stored episodes using a hybrid GLiNER-first pipeline: a local NER model (GLiNER2) identifies named entities first; the LLM is only called for claim extraction when ≥2 entities are found, reducing LLM calls by ~40–60%. Names are resolved via a 4-tier cascade (exact → alias → fuzzy → embedding) and concept nodes with typed relation edges are built automatically. Pronoun entities are merged into named-entity atoms via alias lookup or GLiNER2 classification. Before each LLM extraction call, Smrti queries the most salient named entities already in memory (persons, organizations, projects, tools, locations, events, goals) and injects them as context so the LLM can resolve pronouns and vague references ("I" → person, "we" → organization, "the project" → project name) even across sessions. This is on by default and can be disabled with `SMRTI_EXTRACT=0`. Three extraction modes are available via `SMRTI_EXTRACT_MODE`: `hybrid` (default), `llm` (LLM-only, backward-compatible), and `local` (no LLM).
